...
// src/lang/index.ts
var currentLang = ref(en_default);
...
function updateLang2(lang) {
  if (lang === "zh") {
    currentLang.value = zh_default;
  } else if (lang === "en") {
    currentLang.value = en_default;
  }
}
globalThis.t = t2;
globalThis.updateLang = updateLang2;
...
async function selectOperationType() {
  const { operationType } = await (0, import_prompts.default)(
    [
      {
        name: "operationType",
        type: "select",
...
async function configCreateArgs() {
  const { template, folderName, prefix, branchName } = await (0, import_prompts.default)(
    [
      {
        name: "template",
        type: "select",
